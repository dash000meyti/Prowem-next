...
- Fonts: `src/fonts/` (local faces + catalog; `fontVariablesClassName` on product and `/dev` `<html>`)
...
- Product `<html lang dir>` + theme CSS vars + font CSS vars: `src/app/[lang]/layout.tsx`. Optional lab layout: `src/app/dev/layout.tsx` (English, playground LTR/RTL, no dark mode)
