...
`CardHeader` / `CardFooter` `variant`: `none` | `filled` (`bg-panel-hover/50`) | `border` (default; header `shadow-edge-bottom`, footer `shadow-edge-top`) | `divider` (`filled` + `border`). Those `border` / `divider` lines use the parent Card’s `border` width and `borderColor`; slots do not take those props. `CardContent` has no variant yet.
