...
- Shared scales: `theme.colors` (primary, surface, panel, accent 1–4, success, warning, error — action colors are `base` + `foreground` + `hover` + `glow` + `shadow`; panel is `base` + `foreground` + `hover`), `theme.radius` (`none`, `xs`, `sm`, `md`, `lg`, `xl`, `full`), `theme.borderWidth` (`none`–`lg`), `theme.container` (`xs`–`xl` plus `full`)
